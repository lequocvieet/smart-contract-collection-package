...
contract QuadraticVoting is Owned {
    ...
    */
    function makePoll(
        ...
        */
        ...
        require(
            _firstVoteCost >= 1,
            "First vote cost must be greater than 1 Wei"
        );
    ...
    function addCandidate(
        uint256 _pollId,
        bytes32 _name
    ) public onlyAdmin(_pollId) createdOrCommitPhase(_pollId) stopInEmergency {
        // SafeMath8 also catches this require(), but this allows an error message to be provided
        require(
            polls[_pollId].candidateCount < 255,
            "Can not add more than 255 candidates."
        );
    ...
    function transferAdmin(
        uint256 _pollId,
        address _newAdmin
    ) public stopInEmergency onlyAdmin(_pollId) returns (address) {
    ...
    */
    function completePoll(
        uint256 _pollId
    ) public completePhase(_pollId) returns (uint256) {
        uint256 topChoice;
        uint256 donationAmount = polls[_pollId].totalWeiPaid -
            polls[_pollId].totalWeiRefunded;
        ...
        returns (bool)
    {
        ...
        uint256 voteCost = (_numVotes * (_numVotes)) *
            (polls[_pollId].firstVoteCost);
        ...
        polls[_pollId].totalVotesCommitted =
            polls[_pollId].totalVotesCommitted +
            (_numVotes);
        ...
        returns (uint256)
    {
        ...
        bytes32 revealHash = keccak256(
            abi.encodePacked(_pollId, uint256(numVotes), _vote, _salt)
        );
        require(
        ...
        uint256 _voteCount = uint256(
            polls[_pollId].candidates[_vote].voteCount
        );
        ...
        uint256 voteRevealerRefund = (polls[_pollId].totalWeiPaid) /
            (polls[_pollId].totalVotesCommitted);
    ...
    ) public view returns (uint32 voteCount, bytes32 name) {
