...
contract CryptoPunksMarket {
    ...
    //note this balance tell how many punks each address is owning
    ...
    function setInitialOwner(address to, uint punkIndex) public {
        if (msg.sender != owner)
            revert("must be contract owner to set initial");
    ...
    function allInitialOwnersAssigned() public {
        if (msg.sender != owner)
            revert("must be contract owner to assign all punks");
    ...
    function getPunk(uint punkIndex) public {
        ...
        if (punkIndexToAddress[punkIndex] != address(0))
            revert("Punk has claimed before");
        if (punkIndex >= 10000) revert("index out of range punks");
    ...
    function buyPunk(uint punkIndex) public payable {
        ...
        if (offer.seller != punkIndexToAddress[punkIndex])
            revert("Seller no longer owner of punk"); // Seller no longer owner of punk
    ...
    function enterBidForPunk(uint punkIndex) public payable {
        ...
        if (!allPunksAssigned) revert("All punks must be assigned");
        if (punkIndexToAddress[punkIndex] == address(0))
            revert("Unclaimed punk");
        if (punkIndexToAddress[punkIndex] == msg.sender)
            revert("Cannot bid your own punk");
        if (msg.value == 0) revert("Cannot bit with zero value");
        Bid storage existing = punkBids[punkIndex];
        if (msg.value <= existing.value)
            revert("Need to bid with higher price");
        if (existing.value > 0) {
    ...
    function acceptBidForPunk(uint punkIndex, uint minPrice) public {
        ...
        Bid storage bid = punkBids[punkIndex];
        if (bid.value == 0) revert("punk has no bid");
    ...
    function withdrawBidForPunk(uint punkIndex) public {
        ...
        if (punkIndexToAddress[punkIndex] == address(0))
            revert("Unclaimed punk");
        if (punkIndexToAddress[punkIndex] == msg.sender)
            revert("Cannot withdraw bid your own punk");
        Bid storage bid = punkBids[punkIndex];
    ...
    function getPunkBids(uint256 punkIndex) public view returns (Bid memory) {
        return punkBids[punkIndex];
    }

    function getPendingWithdrawals(address bidder) public view returns (uint) {
        return pendingWithdrawals[bidder];
    }

    function getAddressByPunkIndex(
        uint256 punkIndex
    ) public view returns (address) {
        return punkIndexToAddress[punkIndex];
    }

    function getPunkOfferForSale(
        uint256 punkIndex
    ) public view returns (Offer memory) {
        return punksOfferedForSale[punkIndex];
    }

    function getAllPunksAssignedValue() public view returns (bool) {
        return allPunksAssigned;
    }

    function getPunkRemainToAssign() public view returns (uint) {
        return punksRemainingToAssign;
    }

    function getNumberPunkOfAddress(
        address user
