...
contract Median is LibNote {
    ...
    function poke(
        ...
        for (uint i = 0; i < val_.length; i++) {
            ...
            uint8 sl = uint8(explicitConvert(signer) >> 152);
    ...
    function lift(address[] calldata a) external note auth {
        for (uint i = 0; i < a.length; i++) {
            ...
            uint8 s = uint8(explicitConvert(a[i]) >> 152);
    ...
    function drop(address[] calldata a) external note auth {
        for (uint i = 0; i < a.length; i++) {
            orcl[a[i]] = 0;
            slot[uint8(explicitConvert(a[i]) >> 152)] = address(0);
        }
    }
    ...
    function explicitConvert(address a) internal pure returns (uint256) {
        return uint256(uint160(a));
    }

    function getOrcl(address a) public view returns (uint256) {
        return orcl[a];
    }
    ...
    function testHash(
        ...
        return keccak256(abi.encodePacked(val_, age_, wat));
    }
