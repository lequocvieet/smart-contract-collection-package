...
contract Vat {
    ...
    function file_Line(bytes32 what, uint data) external auth {
        require(live == 1, "Vat/not-live");
    ...
    function slip(bytes32 ilk, address usr, int256 wad) external auth {
        gem[ilk][usr] = _add(gem[ilk][usr], wad); //add collateral
    }
    ...
    function frob(
        ...
        address v, //user v
        address w, //user w
        int dink, //change in collateral
        int dart //change in debt
    ) external {
        // system is live
        require(live == 1, "Vat/not-live");
    ...
    function getIlkSpotPrice(bytes32 ilk) public view returns (uint256) {
        return ilks[ilk].spot;
    }
