...
contract Jug {
    ...
    function file_base(bytes32 what, uint data) external auth {
    ...
    function drip(bytes32 ilk) external returns (uint rate) {
        require(now >= ilks[ilk].rho, "Jug/invalid-now"); //rho is Time of last drip [unix epoch time]
        (, uint prev) = vat.ilks(ilk);
        console.log("prev", prev); //rate initial

        console.log("base+duty", _add(base, ilks[ilk].duty));
        console.log(
            "pow",
