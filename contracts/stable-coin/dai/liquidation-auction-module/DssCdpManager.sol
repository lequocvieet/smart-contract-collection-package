...
contract DssCdpManager {
    ...
    function give(uint cdp, address dst) public cdpAllowed(cdp) {
    ...
    function frob(uint cdp, int dink, int dart) public cdpAllowed(cdp) {
        address urn = urns[cdp];
        VatLike(vat).frob(ilks[cdp], urn, urn, urn, dink, dart);
    }

    // Transfer wad amount of cdp collateral from the cdp address to a dst address.
    function flux(uint cdp, address dst, uint wad) public cdpAllowed(cdp) {
    ...
    function move(uint cdp, address dst, uint rad) public cdpAllowed(cdp) {
    ...
    function getUrn(uint cdpID) public view returns (address) {
        return urns[cdpID];
    }
}
