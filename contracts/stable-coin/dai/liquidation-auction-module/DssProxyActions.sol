...
contract DssProxyActions is Common {
    ...
    function _getDrawDart(
        ...
        console.log("dai balance in vat", dai);
        ...
        if (dai < mul(wad, RAY)) {
            ...
            console.log("dart", sub(mul(wad, RAY), dai) / rate);
...
contract DssProxyActionsEnd is Common {
    ...
    function freeGem(
        ...
        uint amt = _free(manager, end, cdp) /
    ...
    function cashGem(
        ...
        EndLike(end).cash(ilk, wad);
        // Exits token amount to the user's wallet as a token
        uint amt = mul(wad, EndLike(end).fix(ilk)) /
            RAY /
            10 ** (18 - GemJoinLike(gemJoin).dec());
        GemJoinLike(gemJoin).exit(msg.sender, amt);
    }
