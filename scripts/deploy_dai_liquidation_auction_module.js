...
async function main() {
  ...
  await median
    .connect(account0)
    .lift([account1.address, account2.address, account3.address]);
  ...
  console.log("type of price type", typeof priceType);
  let hash1 = ethers.utils.solidityKeccak256(
  ...
  let hash2 = ethers.utils.solidityKeccak256(
    ["uint256", "uint256", "bytes32"],
    [ethers.utils.parseEther("450"), 1004, priceType]
  );
  ...
  //Hash
  let hash3 = ethers.utils.solidityKeccak256(
    ["uint256", "uint256", "bytes32"],
    [ethers.utils.parseEther("500"), 1009, priceType]
  );
  ...
  console.log(
    " New Update oracle Price:   Next oracle Price",
    newOraclePrice[0]
  );
  ...
  DSToken = await hre.ethers.getContractFactory(
    "contracts/stable-coin/dai/liquidation-auction-module/token.sol:DSToken"
  );
  ...
  Dai = await hre.ethers.getContractFactory(
    "contracts/stable-coin/dai/liquidation-auction-module/token.sol:DSToken"
  );
  ...
  GemJoin = await hre.ethers.getContractFactory(
    "contracts/stable-coin/dai/liquidation-auction-module/join.sol:GemJoin"
  );
  ...
  DaiJoin = await hre.ethers.getContractFactory(
    "contracts/stable-coin/dai/liquidation-auction-module/join.sol:DaiJoin"
  );
  ...
  let decimal_Line = ethers.BigNumber.from(
  ...
  await median
    .connect(account0)
    .lift([account10.address, account11.address, account12.address]);
  //Trusted oracle must sign message before verify on-chain
  //val, age,wat
  //oracle 4 price=100
  //oracle 5 price =150
  //oracle 6 price=200
  //Hash
  let hash4 = ethers.utils.solidityKeccak256(
    ["uint256", "uint256", "bytes32"],
  ...
  //Hash
  let hash5 = ethers.utils.solidityKeccak256(
    ["uint256", "uint256", "bytes32"],
    [ethers.utils.parseEther("150"), 200000000000000, priceType]
  );
  ...
  //Hash
  let hash6 = ethers.utils.solidityKeccak256(
    ["uint256", "uint256", "bytes32"],
    [ethers.utils.parseEther("200"), 300000000000000, priceType]
  );
  ...
  await median
    ...
    .poke(
      ...
      [1000000000000000, 200000000000000, 300000000000000],
      [sig4.v, sig5.v, sig6.v],
      [sig4.r, sig5.r, sig6.r],
      [sig4.s, sig5.s, sig6.s]
    );
  ...
  let dust_decimal = ethers.BigNumber.from(
  ...
  let Hole_decimal = ethers.BigNumber.from(
  ...
  await daiJoin
    .connect(account2)
    .join(account2.address, ethers.utils.parseEther("300"));
  ...
  let sump_decimal = ethers.BigNumber.from(
